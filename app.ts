...
function drawTriangle(context: CanvasRenderingContext2D | null, vertices: Point[]) {
  if (!context) {
...
function updateTriangleTable() {
  const triangleTable = document.getElementById('triangleTable') as HTMLTableElement;
...
function handleColorChange(triangleId: string, vertexIndex: number, event: Event) {
...
function editTriangle(triangleId: string) {
  let index = -1;
